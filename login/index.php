<?php
            ...
            if(isset($_POST["submit"])){
                ...
                $sql = "SELECT * FROM user WHERE phone_number = '$phone' AND password = '$pass'";
                ...
                if(mysqli_num_rows($query) > 0){
                    ...
                    //error
                    TextNode("error", "Unknown login details");
                } 
            }
            ...
            function TextNode($classname, $msg){
                $element ="<div class='$classname absolute bottom-0 w-64 m-8 p-8 h-16 bg-red-400 shadow shadow-lg font-bold text-white rounded'>$msg</div>";
                echo $element;
            }
...
<script>
    ...
    <nav class='w-screen p-4 inline-flex shadow' style='background: #f2fbfe'>
        ...
        <div class="relative my-4 font-bold mx-64" style='color:#68696a'>
            ...
            <a href='../register/' class='mx-4' ><i class ='fa fa user'> </i> Register</a>
            <a href='/' class='mx-4 text-red-400'>Login</a>
        </div>
    </nav>
    <div class='w-1/2 bg-white m-auto shadow shadow-lg relative inline-flex left-1/4 border rounded my-8'>
        ...
        <div class='w-1/2 bg-white p-8'>
            <h1 class='text-gray-400 text-2xl'>Login</h1>
				<form class="" action="" method="POST">		
				<input class="block border rounded shadow-inner text-gray-400 p-4 my-4 w-full phoneNumber" type="text" name="phone_number" placeholder="Phone number (0888 800 900 format)" minlength='10' maxlength='14' required>
				<input class="block border rounded shadow-inner text-gray-400 p-4 my-4 w-full password" type="password" name="password" placeholder="Password" min='8' max='50' required>
						<button class="form rounded border bg-blue-500 p-2 w-1/2 text-white py-4 shadow rounded" name="submit">
							Login
						</button>
            </div>
            </div>
        ...
        <script>
            $('.form').click(function(event){
                ...
                if(phoneNumber && password){
                    if(Validator.isValidContact(phoneNumber) && Validator.isValidPassword(password)){
                        return true;
                    }
                }
                else{
